...
class GoalDecomposer @Inject constructor() {
    ...
    private fun decomposeMedia(cmd: ParsedCommand): AgentGoal {
        ...
        steps += SubGoal("$appName launch",
            AgentAction.LaunchApp(pkg, appName),
            expectedOutcome = pkg, critical = true, waitAfterMs = 3500)
        steps += SubGoal("Wait load", AgentAction.Wait(1500), waitAfterMs = 0)

        if (query != null) {
            val searchBarTarget = if (pkg.contains("youtube"))
            ...
            steps += SubGoal("Search bar tap", AgentAction.Tap(searchBarTarget), waitAfterMs = 1000)
            steps += SubGoal("Type: $query",
                AgentAction.TypeText(query, clearFirst = true),
                expectedOutcome = query, critical = true, waitAfterMs = 600)
            steps += SubGoal("Search execute", AgentAction.PressSearch, waitAfterMs = 3000)

            if (shouldPlay) {
                steps += SubGoal("Wait results", AgentAction.Wait(1000), waitAfterMs = 0)
                val firstWord = query.split(" ").firstOrNull { it.length > 2 }
                steps += SubGoal("Pehla result play",
                    AgentAction.Tap(UiTarget(primaryText = firstWord, role = UiRole.LIST_ITEM)),
                    waitAfterMs = 1000)
            }
        }

        return AgentGoal(description = "$appName: $query", subGoals = steps,
            targetPkg = pkg,
    ...
    private fun decomposeMessage(cmd: ParsedCommand): AgentGoal {
        ...
        if (contact != null) {
            steps += SubGoal("Search bar",
                AgentAction.Tap(UiTarget(primaryText = "Search", role = UiRole.SEARCH_BAR)),
                waitAfterMs = 800)
            steps += SubGoal("Contact: $contact",
                AgentAction.TypeText(contact, clearFirst = true),
                expectedOutcome = contact, critical = true, waitAfterMs = 1800)
            steps += SubGoal("Contact tap",
                AgentAction.Tap(UiTarget(primaryText = contact)),
                critical = true, waitAfterMs = 1500)
        }

        if (message != null) {
            steps += SubGoal("Message box",
                AgentAction.Tap(UiTarget(primaryText = "Type a message",
                    hint = "Type a message", role = UiRole.TEXT_FIELD)),
                waitAfterMs = 800)
            steps += SubGoal("Message: ${message.take(20)}",
                AgentAction.TypeText(message, clearFirst = false),
                expectedOutcome = message, critical = true, waitAfterMs = 600)
            steps += SubGoal("Send",
                AgentAction.Tap(UiTarget(role = UiRole.SEND_BUTTON, primaryText = "Send")),
                critical = true, waitAfterMs = 500)
        }

        return AgentGoal(description = "$appName: $contact", subGoals = steps,
            targetPkg = pkg, expectedFinalState = "Message sent")
    }
    ...
    private fun decomposeOpenApp(cmd: ParsedCommand): AgentGoal {
        ...
        return AgentGoal(description = "$name open",
            subGoals = listOf(SubGoal("$name launch",
                AgentAction.LaunchApp(pkg, name),
                expectedOutcome = pkg, critical = true, waitAfterMs = 3000)),
            targetPkg = pkg)
    }
    ...
    private fun simple(desc: String, action: AgentAction) = AgentGoal(
        description = desc,
        subGoals = listOf(SubGoal(desc, action, critical = true))
    )
